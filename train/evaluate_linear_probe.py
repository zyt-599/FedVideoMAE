...
def _threshold_scan(labels_np, probs_np, metric='f1_macro'):
    from sklearn.metrics import precision_recall_fscore_support, confusion_matrix
    best_thr = 0.5
    best_metric = -1
    best_stats = None
    p1 = probs_np[:, 1]
    for thr in [x/100 for x in range(10, 91)]:  # 0.10..0.90
...
def _probs_with_tta(model, x, tta_hflip: bool = False, temporal_clips: int = 1, deterministic_temporal: bool = False):
    with torch.no_grad():
        all_probs = []
        ...
        for k in range(temporal_clips):
            ...
            if temporal_clips > 1 and xk.size(1) > 1:
                if deterministic_temporal:
                    ...
                    shift = int(torch.randint(low=0, high=xk.size(1), size=(1,), device=xk.device).item())
...
def evaluate_classification_balanced(model, dataloader, device, num_samples=400, threshold_scan=False, decision_threshold: float = None, tta_hflip: bool = False, temporal_clips: int = 1, deterministic_temporal: bool = False, scan_metric: str = 'f1_macro') -> Dict[str, float]:
    from sklearn.metrics import precision_recall_fscore_support, confusion_matrix
    import random
    model.eval()
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            ...
            prob = _probs_with_tta(model, x, tta_hflip=tta_hflip, temporal_clips=temporal_clips, deterministic_temporal=deterministic_temporal)
    ...
    if threshold_scan and probabilities:
        ...
        thr, _, stats = _threshold_scan(labels_np, probs_np, metric=scan_metric)
...
def evaluate_classification(model, dataloader, device, num_samples=50, threshold_scan=False, decision_threshold: float = None, tta_hflip: bool = False, temporal_clips: int = 1, deterministic_temporal: bool = False, scan_metric: str = 'f1_macro') -> Dict[str, float]:
...
def main(args):
    ...
    try:
        if getattr(args, 'use_checkpoint_threshold', False) and isinstance(checkpoint, dict) and args.threshold is None and ('best_threshold' in checkpoint):
            bt = checkpoint.get('best_threshold')
            if isinstance(bt, (int, float)):
                args.threshold = float(bt)
                tag = checkpoint.get('best_threshold_metric', 'f1_macro')
                print(f"[Eval] Using checkpoint best_threshold={args.threshold:.2f} (metric={tag})")
    except Exception:
        pass
    ...
    if args.model_type == 'small_head' and (has_bb or has_head):
        ...
        # If LoRA is used but rank unspecified or mismatched, override with inferred rank
        inferred_r = _infer_lora_r(state_dict)
        if peft_cfg and peft_cfg.get('use_lora', False) and inferred_r:
            peft_cfg = dict(peft_cfg)  # shallow copy
            peft_cfg['lora_r'] = inferred_r
            # Heuristic: scale alpha with rank if not provided
        ...
        head = MLPHead(in_dim, head_cfg.get('hidden_dim', 512), head_cfg.get('num_classes', 2))
    ...
    def fix_state_dict_keys(state_dict):
        ...
        for key, value in state_dict.items():
            ...
            if key.startswith('bb.'):
                key = key[len('bb.'):]
            # skip recon_prompt / decoder / LoRA A,B
            if 'recon_prompt' in key or '.decoder.' in key or key.endswith('.A') or key.endswith('.B'):
                continue
            # map base weights to plain
